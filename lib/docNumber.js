...
export function bidangCode(bidang) {
  return BIDANG_CODE[bidang] || String(bidang || "UMUM").slice(0, 3).toUpperCase();
}
...
export async function generateDocNumber(env, bidang, effectiveDate) {
  ...
  return `${seqStr}/SOP-${bidangCode(bidang)}/SMK.MUHADA/${romanMonth(month)}/${year}`;
}
